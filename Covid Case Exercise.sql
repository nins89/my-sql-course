...
SELECT
	...
	, cc.DateRecorded
	, cc.DailyCases
	, SUM(cc.dailycases) OVER (partition BY cc.country order by daterecorded ) AS CumulativeCases
FROM
	CovidCase cc
...
WHERE
    Ranking <= 3
	order by country, dailycases desc
...
FROM
    CovidCase cc
WHERE
    cc.DateRecorded <= '2020-03-15' -- Filter for dates on or before March 15th, 2020
ORDER BY
    cc.Country,
    cc.DateRecorded;